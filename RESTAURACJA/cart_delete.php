    ...
    if(!isset($_SESSION['zalogowany']))
    {
        ...
        try
        {
            ...
            else
            {
                ...
                $result = $connected->query("SELECT * FROM cart WHERE `cart`.`LoginID` = '$loginId' and `cart`.`ProductID` = '$productID';");
                if (!$result) throw new Exception($connected->error);
                $product = mysqli_fetch_all($result,MYSQLI_ASSOC);
                $how_many = $product[0]["quantity"];
                $_SESSION['cart_capacity'] -= $how_many;
